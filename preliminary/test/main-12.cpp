    ...
    for(int i = 0; i < n_r; i++){
        ...
        if (robots[i].running) {
        	if (robots[i].good_taken == -1){
				...
				for(Good g: goods) {
					if (available(g, frame_id) && (dis2g[g.id][x][y] <= 100 || dis2g[g.id][x][y] <= inf && robots[i].target_goods.size() == 0)) // distance too large: A HYPERPARAMETER TO SWITCH
...
void boat_dispatch(int frame_id){
	...
	for(i = 0; i < n_bo; i++){
		...
		|| frame_id + berths[boats[i].pos].ttime > max_ending_time) {
			...
			if (frame_id < 12500 || frame_id >= 12500 && (frame_id + berths[boats[i].pos].ttime >= max_ending_time || boats[i].load == boats[i].cap)) {
				// cerr << "boat " << i << ", berth " << boats[i].pos << " to berth -1,  @ frame " << frame_id << ": " << boats[i].load << "/" << boats[i].cap << endl;
				berths[boats[i].pos].occupied = -1;
				berths[boats[i].pos].reserved = -1;
				boats[i].status = 0;
				boats[i].pos = -1;
