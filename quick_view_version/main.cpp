...
void init() {
    ...
    // map-oriented programming -lol-
    if (env[197][10] == 'T') GT = 1;
    else {
        if (env[197][197] == 'T') GT = 2;
        else GT = 3;
    }
...
void input_frame(int frame_id) {
    ...
    for (auto &robot: robots) {
        ...
        if (robot.good_taken.size() < robot.volumn) {
            ...
            sort(robot.target_goods.begin(), robot.target_goods.end(), [x, y](int e, int f) {
                return goods[e].value / (1.0 + goods[e].dis[x][y]) > goods[f].value / (1.0 + goods[f].dis[x][y]);
            });
...
void robot_dispatch(int frame_id) {
    ...
    for (auto &robot: robots) {
        ...
        pull(robot, goods_id, berth_id);
		if (goods_id.size() > 0) {
            for (int i = goods_id.size() - 1; ~i; i--) {
                sum_value += goods[goods_id[i]].value;
                berths[berth_id].received_goods.push(goods_id[i]);
            }
			printf("pull %d\n", robot.id);
			fflush(stdout);
			continue;
...
void labour_purchase(int frame_id) {
    int max_robot, max_boat;
    if (GT == 1) max_robot = 10, max_boat = 3;
    else if (GT == 2) max_robot = 12, max_boat = 1;
    else if (GT == 3) max_robot = 12, max_boat = 1;
    if (frame_id - last_purchase_robot > 10 && robots.size() < max_robot) {
        ...
        for (auto pos: robot_spawn_points) {
            if (which_robot[pos.x][pos.y] == -1) {
                lbot(pos, 0); // all for type 0 robots
                break;
            }
        }
    }
    if (frame_id - last_purchase_boat > 2000 && money >= constants::boat_cost && boats.size() < max_boat) {
