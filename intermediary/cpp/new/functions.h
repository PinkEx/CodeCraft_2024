...
double eval_goods(Position ori, Position pos, std::vector <int> target_goods, int matched_good) {
    // return good_id == -1 ? 0 : 1.0 / (goods[good_id].dis[pos.x][pos.y] + 0.5) - 1.0 / (goods[good_id].dis[ori.x][ori.y] + 0.5);
	double score = 0, coe, delta; int order = 0;
    if (~matched_good) {
        delta = 1.0 / (goods[matched_good].dis[pos.x][pos.y] + 0.5)
                - 1.0 / (goods[matched_good].dis[ori.x][ori.y] + 0.5);
        coe = G;
    } else {
        coe = 1;
    }
	...
	for (auto good_id: target_goods) {
        if (good_id == matched_good) continue;
...
double move(const Robot &r, int d) {
    ...
    if (~r.good_taken) {
        ...
        score = eval_goods(r.pos, Position(x, y), r.target_goods, r.matched_good);
    }
