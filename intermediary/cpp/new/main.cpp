...
void input_frame(int frame_id) {
    ...
    for (int i = 0; i < k; i++) {
        ...
        if (v == 0) {
            ...
            // if (v < 20) continue;
            all_value += v;
            goods.emplace_back(Good(id, Position(x, y), v, frame_id));
            which_good[x][y] = id;
    ...
    for (auto &robot: robots) {
        ...
        if (robot.good_taken == -1) {
            ...
            sort(robot.target_goods.begin(), robot.target_goods.end(), [x, y, frame_id](int e, int f) {
                // standard 1: dis
                // return goods[e].dis[x][y] < goods[f].dis[x][y];
                // standard 2: value / dis
                return (goods[e].value / 101 + 2) / (1.0 + goods[e].dis[x][y]) > (goods[f].value / 101 + 2) / (1.0 + goods[f].dis[x][y]);
            });
            if (robot.matched_good == -1) assign_needed.emplace_back(robot.id);
        } else {
    ...
    assigned.resize(assign_needed.size(), false);
    // std::set < std::pair < int, std::pair < int, int > > > top_scores;
    std::set < std::pair < double, std::pair < int, int > > > top_scores;
    for (int t = 0; t < assign_needed.size(); t++) {
        top_scores.clear();
        for (auto i: assign_needed) {
            if (assigned[i]) continue;
            for (auto good_id: robots[i].target_goods) {
                if (~goods[good_id].pursuer) continue;
                // int score = goods[good_id].dis[robots[i].pos.x][robots[i].pos.y];
                double score = -(goods[good_id].value / 101 + 2) / (1.0 + goods[good_id].dis[robots[i].pos.x][robots[i].pos.y]);
                top_scores.insert(std::make_pair(score, std::make_pair(i, good_id)));
            }
        }
        if (top_scores.empty()) break;
        auto i = (*top_scores.begin()).second.first;
        auto good_id = (*top_scores.begin()).second.second;
        goods[good_id].pursuer = i;
        robots[i].matched_good = good_id;
        assigned[i] = true;
    }
...
void labour_purchase(int frame_id) {
    if (frame_id - last_purchase_robot > 5 && money >= constants::robot_cost && robots.size() < 16) {
    ...
    if (frame_id - last_purchase_boat > 10000 && money >= constants::boat_cost && boats.size() < 2) {
...
int main() {
    ...
    while (scanf("%d", &frame_id) != EOF) {
        ...
        if (frame_id == 14999) {
            ...
            std::cerr << "rest_all_value = " << all_value + 25000 - robots.size() * constants::robot_cost - boats.size() * constants::boat_cost << std::endl;
            std::cerr << "sum_value = " << sum_value << std::endl;
            std::cerr << "rest_sum_value = " << sum_value + 25000 - robots.size() * constants::robot_cost - boats.size() * constants::boat_cost << std::endl;
        }
    }
    return 0;
}