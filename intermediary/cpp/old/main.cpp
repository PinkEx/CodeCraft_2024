...
void input_frame(int frame_id) {
    ...
    for (auto &robot: robots) {
        ...
        if (robot.good_taken.size() < robot.volumn) {
            ...
            sort(robot.target_goods.begin(), robot.target_goods.end(), [x, y](int e, int f) {
                return (goods[e].value / 40) / (1.0 + goods[e].dis[x][y]) > (goods[f].value / 40) / (1.0 + goods[f].dis[x][y]);
            });
    ...
    // std::cerr << frame_id << " !\n";
    // for (auto &robot: robots) {
    //     std::cerr << robot.id << ' ' << robot.matched_good << std::endl;
    // }
...
void robot_dispatch(int frame_id) {
    ...
    for (auto &robot: robots) {
		...
		for (int d = 0; d < 4; d++) {
            ...
            // std::cerr << robot.id << ' ' << d << ' ' << esti_value << "\n";
    ...
    if (frame_id - last_purchase_robot > 10 && robots.size() < max_robot) {
        ...
        for (auto pos: robot_spawn_points) {
            if (which_robot[pos.x][pos.y] == -1) {
                lbot(pos, 0);
                // if (robots.size() < 8) lbot(pos, 0); else lbot(pos, 1);
                break;
            }
        }
    }
...
int main() {
    ...
    while (scanf("%d", &frame_id) != EOF) {
        ...
        // if (frame_id == 14999) {
        //     std::cerr << "max_value = " << sum_value - 25000 << std::endl;
